#Test for excess significance in Gynecological cancers
...
#BREAST OS
#find the observed number of positive studies for breast-os
O_breast_os <- subset(dat, Site=="Breast" & Outcome=="OS", 
                      select = c(sig_O, largest_power))
table(O_breast_os$sig_O)
#find the expected number of positive studies for breast-os
sum(O_breast_os$largest_power)

#BREAST DFS
#find the observed number of positive studies for breast-dfs
O_breast_dfs <- subset(dat, Site=="Breast" & Outcome=="DFS", 
                       select = c(sig_O, largest_power))
table(O_breast_dfs$sig_O)
#find the expected number of positive studies for breast-os
sum(O_breast_dfs$largest_power)

#GYNECOLOGIC OS
O_gyn_os <- subset(dat, Site=="Gynecologic" & Outcome=="OS", 
                   select = c(sig_O, largest_power))
table(O_gyn_os$sig_O)
#find the expected number of positive studies for breast-os
sum(O_gyn_os$largest_power)
